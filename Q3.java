...
public class Q3 {
	...
	static long actualCalc(int[] a, int lo, int hi) {
		int NumOfMathPowers=0;
		if (lo == hi) { 
			NumOfMathPowers+=2;
			return power(a[lo], 2) + power(a[hi],2);
		}
		NumOfMathPowers++;
		int m = (lo + hi) / 2;
		long totalMathPowers = power(lo, 2) + proc(a, lo, m);
		while (hi > lo) {
			NumOfMathPowers++;
			totalMathPowers += power(a[lo], 2);
			lo+=2;
		}
		
		return NumOfMathPowers;
	}
	...
	static long model(int n) {
		double predictedMathPower = Math.log(n)/Math.log(2) + n + 1;
        return (long)predictedMathPower;
	}
	...
	public static void main(String[] args) {
		...
		for (int n = 2; n <= 4096; n *= 2) {
			...
			long predicted=model(n);
			long actual= actualCalc(a,0,n-1);
			 
			System.out.println(String.format("%6d\t%12d\t%10d\t%11d", n, val, actual, predicted));
		}
	}
}
